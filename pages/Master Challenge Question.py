import streamlit as st

st.title("Master Challenge Question")

col1, col2,col3=st.columns(3)
with col1:
    st.image(r"image/Master Challenge Question/pic1.png")
    st.image(r"image/Master Challenge Question/pic4.png")
    st.image(r"image/Master Challenge Question/pic7.png")
with col2:
    st.image(r"image/Master Challenge Question/pic2.png")
    st.image(r"image/Master Challenge Question/pic5.png")
with col3:
    st.image(r"image/Master Challenge Question/pic3.png")
    st.image(r"image/Master Challenge Question/pic6.png")

